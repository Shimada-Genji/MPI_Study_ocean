...
!**************************************************************
    ...
    if (myrank.eq.0) write (*,*) "[INIT] Initializing data..."
...
! Would not counted in to the calculation time.
    ...
    if (myrank.eq.0) write (*,*) "[INIT] process 0 has initilized data, start to distributed"
    if (myrank.eq.0) write (*,*)
    call distribute_init_data(myrank,ierr) ! Not implemented.
    if (myrank.eq.0) write (*,*) "[DISTRIBUTE] Done!"
    if (myrank.eq.0) write (*,*)
...
! If programmer needs to use distributed version, just use this style function call.
    call mpdata_adiff_tile_mpi(LBi, UBi, LBj, UBj, MYDATA % oHz, MYDATA % Huon, MYDATA % Hvom, MYDATA % W, MYDATA % Ta,&
     MYDATA % Uind, MYDATA % Dn, MYDATA % Dm, MYDATA % Ua,myrank,ierr) ! Not implemented.
...
! Would not counted in to the calculation time.
  if (myrank.eq.0) write (*,*) "[GATHER] Start gather."
    call gather_data(myrank,ierr,MYDATA % Ua,LBi, UBi, LBj, UBj,N)
